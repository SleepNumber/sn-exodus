...
__webpack_require__.d(__webpack_exports__, {
  getAssetCoverSize: () => (/* binding */ getAssetCoverSize),
  getCloudinaryUrl: () => (/* binding */ getCloudinaryUrl),
  getCloudinaryVersion: () => (/* binding */ getCloudinaryVersion),
  getOptimizedVideo: () => (/* binding */ getOptimizedVideo),
  stripCloudinaryUrl: () => (/* binding */ stripCloudinaryUrl)
});

// UNUSED EXPORTS: asJpg, asMp4, base64GifToSrc, buildSources, filterAssetsByTags, findAssetByTags, findImageByTags, findImagesByTags, findVideoByTags, findVideosByTags, getBgImages, getImageDimensions, getVideoPlaceholder, getVideoPoster, getVideoThumb, hexToRgb, importAll, isFullscreen, isJpg, isMp4, isSecure, isVideoFullscreen, placehold, retainFormat, rgbToHex, safePause, safePlay, sslUrl, toggleFullscreen, withSpecialtyTag, withoutSpecialtyTag
...
var core_logger = __webpack_require__(5813);
...
var core_MediaSource = __webpack_require__(3027);
// EXTERNAL MODULE: ./src/util/core/tags.js
var tags = __webpack_require__(5567);
...
function getOptimizedVideo(videoUrl, width, keepOriginalWidth) {
  ...
  const sources = formats.map(_ref3 => {
    ...
    return core_MediaSource["default"].of(src, type);
  });
  return sources;
}
...
function getElement(target, container) {
  ...
  try {
    ...
    logger.error('Error getting element', 'target', target, 'container', container, err);
    return null;
  }
}
...
"use strict";
/* unused harmony exports isString, capitalize, titlecase, camelCase, camelToSnake, pascalToSnake, snakeToPascal, mattressCase, optionize, deoptionize, dasherize, undasherize, repeat, wordCount, pad, replaceAt, endsWith, firstWord, uuid, lazyId, bytes, pluralIf, pxToNum, truncate, asBool, removeSpecialCharacters */
/* harmony import */ var _constants__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(3168);
...
/* harmony export */ });
/* unused harmony export Transformers */
/**
...
describe('getCloudinaryUrl generates proper URLs for each environment', () => {
  ...
  test('Local', async () => {
    ...
    constants.isDevelopment.mockImplementationOnce(() => true);
    const url = (0,assets.getCloudinaryUrl)('/dist/images/foobar.jpg');
    expect(url).toEqual('/dist/images/foobar.jpg');
  });
  test('QA', async () => {
    window.location = new URL('https://qa.sleepnumber.com');
    const url = (0,assets.getCloudinaryUrl)('/dist/images/foobar.jpg');
    expect(url).toEqual(`https://res.cloudinary.com/${CLOUD_NAMES.qa}/image/upload/f_auto,q_auto:eco/${(0,assets.getCloudinaryVersion)()}/uploads/dist/images/foobar.jpg`);
  });
  test('Staging', async () => {
    window.location = new URL('https://staging.sleepnumber.com');
    window.sn_globals.config.env = 'staging';
    const url = (0,assets.getCloudinaryUrl)('/dist/images/foobar.jpg');
    expect(url).toEqual(`https://res.cloudinary.com/${CLOUD_NAMES.staging}/image/upload/f_auto,q_auto:eco/${(0,assets.getCloudinaryVersion)()}/uploads/dist/images/foobar.jpg`);
  });
  test('Production', async () => {
    window.location = new URL('https://www.sleepnumber.com');
    const url = (0,assets.getCloudinaryUrl)('/dist/images/foobar.jpg');
    expect(url).toEqual(`https://res.cloudinary.com/${CLOUD_NAMES.prod}/image/upload/f_auto,q_auto:eco/${(0,assets.getCloudinaryVersion)()}/uploads/dist/images/foobar.jpg`);
  });
});
describe('util/assets.js', () => {
  test('getAssetCoverSize', async () => {
    ...
    let cover = await (0,assets.getAssetCoverSize)(asset, div);
    ...
    cover = await (0,assets.getAssetCoverSize)(asset, div);
  ...
  test('stripCloudinaryUrl', async () => {
    const testUrls = ['https://res.cloudinary.com/snbr-qa/video/upload/c_limit,q_auto:eco,w_768,vc_vp9/v1/workarea/content/assets/l3tu9r0otltkseoechnp.mp4', 'https://cdn.sleepnumber.com/video/upload/vc_auto/v1/workarea/content/assets/n0wznr08b72f3oa0lkon.mp4', 'https://cdn.sleepnumber.com/video/upload/ac_none/vc_auto/v1/workarea/content/assets/jdzkftjlc4jxdxcpor0g'];
    const expectedUrls = ['https://res.cloudinary.com/snbr-qa/video/upload/v1/workarea/content/assets/l3tu9r0otltkseoechnp', 'https://cdn.sleepnumber.com/video/upload/v1/workarea/content/assets/n0wznr08b72f3oa0lkon', 'https://cdn.sleepnumber.com/video/upload/ac_none/v1/workarea/content/assets/jdzkftjlc4jxdxcpor0g'];
    const stripped = testUrls.map(assets.stripCloudinaryUrl);
    expect(stripped).toEqual(expectedUrls);
  });
  test('getOptimizedVideo', async () => {
    const testUrls = ['https://res.cloudinary.com/snbr-qa/video/upload/c_limit,q_auto:eco,w_768,vc_vp9/v1/workarea/content/assets/l3tu9r0otltkseoechnp.mp4', 'https://cdn.sleepnumber.com/video/upload/vc_auto/v1/workarea/content/assets/n0wznr08b72f3oa0lkon.mp4', 'https://cdn.sleepnumber.com/video/upload/ac_none/vc_auto/v1/workarea/content/assets/jdzkftjlc4jxdxcpor0g'];
    const optimized = {
      desktop: testUrls.map(url => (0,assets.getOptimizedVideo)(url, 2000)),
      mobile: testUrls.map(url => (0,assets.getOptimizedVideo)(url, 300))
    };
    const {
      desktop,
      mobile
    } = optimized_video_urls_namespaceObject;
  ...
  test('getCloudinaryTransformData', async () => {
    const url = 'https://cdn.sleepnumber.com/video/upload/ar_1,c_fill,h_768,so_auto,co_black,e_colorize:40/f_auto/v1/workarea/catalog/product_videos/lyocell-ultra-sheets-set/gallery_video_1.jpg';
    const expected = ['ar_1', 'c_fill', 'h_768', 'so_auto', 'co_black', 'e_colorize:40', 'f_auto'];
    const data = (0,cloudinary.getCloudinaryTransformData)(url);
    ...
    expect(data.suffix).toBe('/v1/workarea/catalog/product_videos/lyocell-ultra-sheets-set/gallery_video_1.jpg');
  });
  test('setCloudinaryTransforms', async () => {
    const url = 'https://cdn.sleepnumber.com/video/upload/ar_1,c_fill,h_768,so_auto,co_black,e_colorize:40/f_auto/v1/workarea/catalog/product_videos/lyocell-ultra-sheets-set/gallery_video_1.jpg';
    const expected = 'https://cdn.sleepnumber.com/video/upload/so_42,co_red/v1/workarea/catalog/product_videos/lyocell-ultra-sheets-set/gallery_video_1.jpg';
    const actual = (0,cloudinary.setCloudinaryTransforms)(url, ['so_42', 'co_red']);
    expect(actual).toBe(expected);
  });
  test('updateCloudinaryTransforms', async () => {
    const url = 'https://cdn.sleepnumber.com/video/upload/ar_1,c_fill,h_768,so_auto,co_black,e_colorize:40/f_auto/v1/workarea/catalog/product_videos/lyocell-ultra-sheets-set/gallery_video_1.jpg';
    const expected = 'https://cdn.sleepnumber.com/video/upload/ar_1,c_limit,h_768,so_42,co_black,e_colorize:40,f_auto/v1/workarea/catalog/product_videos/lyocell-ultra-sheets-set/gallery_video_1.jpg';
    const actual = (0,cloudinary.updateCloudinaryTransforms)(url, ['so_42', 'c_limit']);
    expect(actual).toBe(expected);
  });
});
})();
