...
/* harmony export */ });
/* unused harmony exports setProp, values, reverse, shallowEqual, shrink, eq, has, extend, type */
/* OBJECT UTILS
   --------------------------------------------------------------- */
...
__webpack_require__.d(__webpack_exports__, {
  W9: () => (/* binding */ addGlobalStyles),
  hQ: () => (/* binding */ addWrapperStyles),
  jY: () => (/* binding */ buildStyleString),
  s_: () => (/* binding */ createWrapperStyles),
  ZP: () => (/* binding */ css),
  J0: () => (/* binding */ styleObjectToCss)
});

;// CONCATENATED MODULE: external "stylis"
const external_stylis_namespaceObject = require("stylis");
// EXTERNAL MODULE: ./src/util/core/cache.js
var cache = __webpack_require__(882);
...
;// CONCATENATED MODULE: ./src/util/ui/css.js





const css_cache = new cache/* default */.Z();
 ...
 * `
 */
function styleObjectToCss(input) {
  if ((0,string/* isString */.HD)(input)) return input;
  const entries = Object.entries(input);
  const styleText = entries.reduce((result, _ref, i) => {
    let [k, v] = _ref;
    const last = i === entries.length - 1;
    return `${result}${(0,string/* camelToKabob */.KY)(k)}: ${v};${last ? '' : ' '}`;
  }, '');
  return styleText;
}
 ...
 * @return {string}
 */
function buildStyleString(input) {
  if (Array.isArray(input)) {
    // Array of <string|object>
    return input.filter(Boolean).map(o => {
      if ((0,object/* isPlainObject */.PO)(o)) return styleObjectToCss(o);
      return o;
    }).join(' ');
  } else if ((0,object/* isPlainObject */.PO)(input)) {
    // style object
    return styleObjectToCss(input);
  } else if ((0,string/* isString */.HD)(input)) {
    // string
    return input;
  }
  // anything else
 ...
 */
function addGlobalStyles(id, css) {
 ...
 */
function createWrapperStyles(styles) {
  let css = buildStyleString(styles);
  if (!css) return null;
  const className = `wrapper-css-${(0,string/* hash */.vp)(css)}`;
  css = css.replaceAll('&', `.${className}`);
  if (css_cache.has(className)) return css_cache.get(className);
  let text = (0,external_stylis_namespaceObject.serialize)((0,external_stylis_namespaceObject.compile)(css), (0,external_stylis_namespaceObject.middleware)([element => {
    ...
    } else if (element.type === 'decl' && element.parent.type === '@media') {
      // Top level (naked) rule inside media query,
      // convert it to a rule with declarations
      element.return = `.${className}{${element.value}}`;
    }
  }, external_stylis_namespaceObject.stringify]));
  if ((0,constants/* isDevelopment */.yG)()) {
  ...
  css_cache.put(className, result);
  return result;
}
 ...
 * );
 */
function addWrapperStyles(css) {
  const {
    className,
    styles
  } = createWrapperStyles(css);
...
var __webpack_exports__addGlobalStyles = __webpack_exports__.W9;
var __webpack_exports__addWrapperStyles = __webpack_exports__.hQ;
var __webpack_exports__buildStyleString = __webpack_exports__.jY;
var __webpack_exports__createWrapperStyles = __webpack_exports__.s_;
var __webpack_exports__default = __webpack_exports__.ZP;
var __webpack_exports__styleObjectToCss = __webpack_exports__.J0;
export { __webpack_exports__addGlobalStyles as addGlobalStyles, __webpack_exports__addWrapperStyles as addWrapperStyles, __webpack_exports__buildStyleString as buildStyleString, __webpack_exports__createWrapperStyles as createWrapperStyles, __webpack_exports__default as default, __webpack_exports__styleObjectToCss as styleObjectToCss };
