...
/* harmony export */   hasUrlParam: () => (/* binding */ hasUrlParam),
/* harmony export */   setUrlHash: () => (/* binding */ setUrlHash)
/* harmony export */ });
...
function getUrlParam(name, url) {
  const href = url || _constants__WEBPACK_IMPORTED_MODULE_0__.win?.location?.href;
...
function hasUrlParam(name, url) {
  const loc = url || _constants__WEBPACK_IMPORTED_MODULE_0__.win?.location;
  if (!loc) return false;
...
function setUrlHash(hash) {
  const url = new URL(_constants__WEBPACK_IMPORTED_MODULE_0__.win?.location);
  url.hash = hash;
  _constants__WEBPACK_IMPORTED_MODULE_0__.win?.history?.pushState({}, '', url);
}
})();
