...
class ProductAsset {
  ...
  static isAssetLike(o) {
    ...
    return (0,_object__WEBPACK_IMPORTED_MODULE_0__.has)(o, props);
  }
}
...
__webpack_require__.d(__webpack_exports__, {
  asJpg: () => (/* binding */ asJpg),
  asMp4: () => (/* binding */ asMp4),
  base64GifToSrc: () => (/* binding */ base64GifToSrc),
  buildSources: () => (/* binding */ buildSources),
  filterAssetsByTags: () => (/* binding */ filterAssetsByTags),
  findAssetByTags: () => (/* binding */ findAssetByTags),
  findImageByTags: () => (/* binding */ findImageByTags),
  findImagesByTags: () => (/* binding */ findImagesByTags),
  findVideoByTags: () => (/* binding */ findVideoByTags),
  findVideosByTags: () => (/* binding */ findVideosByTags),
  ...
  getImageDimensions: () => (/* binding */ getImageDimensions),
  getOptimizedVideo: () => (/* binding */ getOptimizedVideo),
  getVideoPlaceholder: () => (/* binding */ getVideoPlaceholder),
  getVideoPoster: () => (/* binding */ getVideoPoster),
  getVideoThumb: () => (/* binding */ getVideoThumb),
  hexToRgb: () => (/* binding */ hexToRgb),
  importAll: () => (/* binding */ importAll),
  ...
  safePause: () => (/* binding */ safePause),
  safePlay: () => (/* binding */ safePlay),
  sslUrl: () => (/* binding */ sslUrl),
  stripCloudinaryUrl: () => (/* binding */ stripCloudinaryUrl),
  toggleFullscreen: () => (/* binding */ toggleFullscreen),
  withSpecialtyTag: () => (/* binding */ withSpecialtyTag),
...
function getVideoPlaceholder(asset) {
  ...
  if (asset instanceof ProductAsset["default"] || ProductAsset["default"].isAssetLike(asset)) {
    url = asset.url;
  ...
  if (asset instanceof MediaSource["default"]) url = asset.src;
  if (!url.includes('cloudinary.com') && !url.includes('cdn.sleepnumber.com')) {
    // not a cloudinary url
    return result;
  }
  result = (0,cloudinary.updateCloudinaryTransforms)(url, [`so_${time}`]);
...
function getVideoPoster(asset) {
  ...
  result = (0,cloudinary.updateCloudinaryTransforms)(result, ['co_black', 'e_colorize:20']);
  return result;
}
...
function getVideoThumb(video) {
  ...
  const thumb = (0,cloudinary.setCloudinaryTransforms)(url, transforms);
...
function getOptimizedVideo(videoUrl, keepOriginalWidth) {
  ...
  const sources = formats.map(_ref4 => {
    ...
    return MediaSource["default"].of(src, type);
  });
  return sources;
}
...
function filterAssetsByTags(collection, tags, exclude) {
  ...
  const assets = collection.filter(asset => {
    const includes = (0,array.includesAll)(asset.tags || [], include);
    let hasExcluded = false;
    if (exclude) {
      let exclude_these = exclude;
      if (typeof exclude === 'string') exclude_these = [exclude];
      hasExcluded = (0,array.includesAny)(asset.tags, exclude_these);
    }
...
function findAssetByTags(collection, tags, exclude) {
  ...
  const asset = collection.find(asset => {
    ...
    if (exclude) {
      let exclude_these = exclude;
      if ((0,string.isString)(exclude)) exclude_these = [exclude];
      hasExcluded = (0,array.includesAny)(asset.tags, exclude_these);
    }
    return includes && !hasExcluded;
  });
...
function safePlay(media) {
  let onFailure = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : core_function.noop;
  const asset = media?.current ? media.current : media;
  if (!asset) return Promise.reject(new Error('No media asset provided'));
  if (!(0,core_function.isFunc)(asset?.play)) return Promise.reject(new Error('Not playable'));
  const promise = asset.play() || Promise.resolve();
  promise.catch(error => {
    // Autoplay was prevented.
    logger["default"].log('Play was prevented on asset:', asset, 'Error:', error);
    onFailure(error);
  });
  return promise;
}
...
function safePause(media) {
  // Allow React refs
  const asset = media?.current ? media.current : media;
  if ((0,core_function.isFunc)(asset?.pause)) asset.pause();
}
