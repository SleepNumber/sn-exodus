...
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   camelToKabob: () => (/* binding */ camelToKabob),
...
__webpack_require__.d(__webpack_exports__, {
  addGlobalStyles: () => (/* binding */ addGlobalStyles),
  addWrapperStyles: () => (/* binding */ addWrapperStyles),
  buildStyleString: () => (/* binding */ buildStyleString),
  createWrapperStyles: () => (/* binding */ createWrapperStyles),
  "default": () => (/* binding */ css),
  styleObjectToCss: () => (/* binding */ styleObjectToCss)
});
...
const css_cache = new cache["default"]();
...
function styleObjectToCss(input) {
  if ((0,string.isString)(input)) return input;
  const entries = Object.entries(input);
  const styleText = entries.reduce((result, _ref, i) => {
    let [k, v] = _ref;
    const last = i === entries.length - 1;
    return `${result}${(0,string.camelToKabob)(k)}: ${v};${last ? '' : ' '}`;
  }, '');
  return styleText;
}
...
function buildStyleString(input) {
  if (Array.isArray(input)) {
    ...
    return input.filter(Boolean).map(o => {
      if ((0,object.isPlainObject)(o)) return styleObjectToCss(o);
      return o;
    }).join(' ');
  } else if ((0,object.isPlainObject)(input)) {
    // style object
    return styleObjectToCss(input);
  } else if ((0,string.isString)(input)) {
    // string
    return input;
  }
  // anything else
...
function createWrapperStyles(styles) {
  ...
  if (!css) return null;
  const className = `wrapper-css-${(0,string.hash)(css)}`;
